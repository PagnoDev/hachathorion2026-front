...
function hex([hex]: TemplateStringsArray): [number, number, number];
function hex(h: string): [number, number, number];
function hex(h: string | TemplateStringsArray): [number, number, number] {
  const s = typeof h === "string" ? h : h[0];
...
export async function downloadItineraryPDF(result: GeneratedItineraryResult) {
  ...
  const PH = 297;
  const M = 15;
  const CW = PW - M * 2;       // 180 mm content width
  const MAX_Y = PH - M - 14;   // reserve 14 mm at bottom for footer
  ...
  // ---- color helpers ----
  const tc = (h: string) => { const [r,g,b] = hex(h); doc.setTextColor(r, g, b); };
  const fc = (h: string) => { const [r,g,b] = hex(h); doc.setFillColor(r, g, b); };
  const dc = (h: string) => { const [r,g,b] = hex(h); doc.setDrawColor(r, g, b); };
  ...
  function continuationHeader() {
    ...
    doc.text("Lages Smart Tourism", M, M - 2);
  ...
  doc.text("Lages Smart Tourism", M, pos.y + 6);
  ...
  // ---- footers (post-process all pages) ----
  ...
  for (let p = 1; p <= totalPages; p++) {
    ...
    doc.setFont("helvetica", "normal"); doc.setFontSize(6); tc(C.gray);
    doc.text("Lages Smart Tourism — lagessmarthourism.app", M, fy);
    doc.text(`${p} / ${totalPages}`, PW - M, fy, { align: "right" });
  }

  doc.save(`roteiro-lages-${request.arrivalDate}.pdf`);
  ...
  function renderItem(item: ItineraryItemDto) {
    ...
    const cardH =
      ...
      (hasTime  ? 4   : 0) +
      5 +                              // name row
      (hasCats  ? 5   : 0) +          // category badges
      Math.max(1, addrLines) * 4 +    // address (wraps)
      3 * 4 +                         // hours / price / distance
      (hasAlert ? 9   : 0);
    ...
    // info rows
    const rows = [
      { label: "Endereço",   value: item.location.address },
